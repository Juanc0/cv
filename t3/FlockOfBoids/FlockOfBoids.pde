 ...
 * Press 'i' to swith between immediate or held render mode
 * Press 'f' to iterate through mesh representation type
 * Press 'h' to switch polygon mesh
 * Press 'w' to render spline surface
 */
...
boolean immediatRenderMode = true;
boolean meshRepresentationType = true;  //  0 --> VVMesh, 1 --> FVMesh
boolean cube = true;
boolean splineSurface = false; 
...
void initControlPoints(){
  for(int i = 0; i < ids.size(); i++){
    controlPoints.add(flock.get(ids.get(i)).position);
  }
}

ArrayList<Integer> ids(int n){
 ArrayList<Integer> ids = new ArrayList(n + 1);
  int aux;
  // get n points
  for(int i = 0; i <= n; i++){
    if (ids.contains(aux = (int) random(initBoidNum))){
      i--;
    } else {   
      ids.add(aux);
      print(aux + " ");
...
void keyPressed() {
  ...
  case 'v':
    ...
    break;
  case 'i':
    immediatRenderMode = !immediatRenderMode; 
    break;
  case 'f':
    meshRepresentationType = !meshRepresentationType;
    break;
  case 'h':
    cube = !cube;
    break;
